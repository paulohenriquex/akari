...
public class FichaTecnicaService {
    ...
    public FichaTecnicaResponseDTO criar(FichaTecnicaRequestDTO dto) {
        ...
        List<Ingrediente> ingredientes = dto
            .ingredientes()
            .stream()
            .map(ingredienteDTO -> {
    ...
    private FichaTecnicaResponseDTO converterParaResponseDTO(
        FichaTecnica fichaTecnica
    ) {
        List<IngredienteResponseDTO> ingredientesDTO = fichaTecnica
            .getIngredientes()
            .stream()
            .map(ing ->
                new IngredienteResponseDTO(
                    ing.getProduto().getId(),
                    ing.getProduto().getNome(),
                    ing.getPercapita(),
                    ing.getProduto().getMedida(),
